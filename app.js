...
app.get("/api/search", async (request, response) => {
  try {
    const { query } = request;
    let {data} = await axios.get(`https://api.travelpayouts.com/aviasales/v3/prices_for_dates?origin=${query.origin}&destination=${query.destination}&unique=false&sorting=price&direct=false&currency=usd&limit=30&page=1&token=aebed97e88519a5cb765d5c68866a86b`)
